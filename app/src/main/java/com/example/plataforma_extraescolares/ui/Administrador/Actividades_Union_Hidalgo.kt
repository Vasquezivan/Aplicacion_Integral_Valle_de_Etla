        ...
        apiService.getEstudiantesPorActividad(idActividad).enqueue(object : Callback<EstudiantesResponse> {
            override fun onResponse(call: Call<EstudiantesResponse>, response: Response<EstudiantesResponse>) {
                if (response.isSuccessful) {
                    ...
                    Toast.makeText(this@Actividades_Union_Hidalgo, "Error al obtener los estudiantes: ${response.code()}", Toast.LENGTH_SHORT).show()
                }
            }

            override fun onFailure(call: Call<EstudiantesResponse>, t: Throwable) {
                Toast.makeText(this@Actividades_Union_Hidalgo, "Fallo en la conexión: ${t.message}", Toast.LENGTH_SHORT).show()
            }
        })
    }
        ...
        btnGuardar.setOnClickListener {
            ...
            apiService.updateEstudiante(estudiante.id, estudianteActualizado).enqueue(object : Callback<Estudiante> {
                override fun onResponse(call: Call<Estudiante>, response: Response<Estudiante>) {
                    if (response.isSuccessful) {
                        Toast.makeText(this@Actividades_Union_Hidalgo, "Estudiante actualizado correctamente", Toast.LENGTH_SHORT).show()
                        fetchEstudiantes(idActividad)
                        dialog.dismiss()
                    } else {
                        Toast.makeText(this@Actividades_Union_Hidalgo, "Error al actualizar el estudiante: ${response.code()}", Toast.LENGTH_SHORT).show()
                    }
                }
